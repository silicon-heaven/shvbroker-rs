...
pub(crate) fn main() -> shvrpc::Result<()> {
    ...
    let (access, role_access_rules, policies, last_login, sql_connection) = if config.use_access_db {
        ...
        let (sql_connection, access_config, role_access_rules, policies, last_login) = smol::block_on( sql::migrate_sqlite_connection(&sql_config_file, &config.access, &config.policies))?;
        (access_config, role_access_rules, policies, last_login, Some(sql_connection))
    } else {
        (config.access.clone(), parse_config_roles(config.access.roles()), config.policies.clone(), LastLogin::default(), None)
    };
    ...
    let broker_impl = BrokerImpl::new(SharedBrokerConfig::new(config), access, role_access_rules, last_login, policies, command_sender, sql_connection);
