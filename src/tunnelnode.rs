...
use crate::brokerimpl::{state_reader, state_writer, BrokerCommand, NodeRequestContext, SharedBrokerState, TunnelId};
...
impl ShvNode for TunnelNode {
    ...
    fn children(&self, shv_path: &str, broker_state: &SharedBrokerState) -> Option<Vec<String>> {
        let tunnels = state_reader(broker_state).active_tunnel_ids().iter().map(|id| format!("{}", *id)).collect();
        if shv_path.is_empty() {
            Some(tunnels)
    ...
    fn process_request(&mut self, frame: &RpcFrame, ctx: &NodeRequestContext) -> shvnode::ProcessRequestResult {
        ...
        let tunid = frame.shv_path().unwrap_or_default().parse::<TunnelId>().ok();
...
pub(crate) async fn tunnel_task(tunnel_id: TunnelId, request_meta: MetaMap, addr: String, from_broker_receiver: Receiver<ToRemoteMsg>, state: SharedBrokerState) -> shvrpc::Result<()> {
    ...
    to_broker_sender.send(BrokerCommand::TunnelActive(tunnel_id)).await?;
    ...
    task::spawn(async move {
        debug!(target: "Tunnel", "ENTER write task");
