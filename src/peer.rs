...
pub(crate) async fn broker_as_client_peer_loop_with_reconnect(
    ...
    let tls_connector = if config.client.url.scheme() == "ssl" {
        Some(Arc::new(build_tls_connector(&config.client.url)?))
    } else {
        None
    };
...
async fn broker_as_client_peer_loop_from_url(
    ...
    config: BrokerConnectionConfig,
    broker_writer: Sender<BrokerCommand>,
    tls_connector: Option<Arc<futures_rustls::TlsConnector>>,
) -> shvrpc::Result<()> {
    ...
    match scheme {
        "tcp" => {
            ...
            debug!("Connecting to TCP broker peer: {address}");
        ...
        "ssl" => {
            ...
            let address = format!("{host}:{port}");
            debug!("Connecting to SSL broker peer: {address}");
            let connector = tls_connector
                .ok_or("TLS connector not initialized")?;
            let stream = TcpStream::connect(&address).await?;
            let stream = connector
                .connect(futures_rustls::pki_types::ServerName::try_from(host.to_owned())?, stream)
                .await?;
            setup_stream_and_run(peer_id, config, broker_writer, stream).await
        }
        "serial" => {
            let port_name = url.path();
            debug!("Connecting to serial broker peer: {port_name}");
